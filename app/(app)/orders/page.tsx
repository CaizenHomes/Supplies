...
import { MarkOrderedModal } from "./mark-ordered-modal";
import { MarkReceivedModal } from "./mark-received-modal";
import { CancelButton } from "./cancel-button";

const ORDER_STATUSES = ["pending_approval", "in_list", "ordered"] as const;
...
export default async function OrdersPage() {
  ...
  const [{ data: items }, { data: activeProfiles }] = await Promise.all([
    supabase.from("items_detailed").select("*").in("status", ORDER_STATUSES),
    ...
    <section>
      <div className="mb-4">
        ...
        <p className="mt-0.5 text-sm text-text-muted">
          Items awaiting approval, approved to buy, ordered but not yet received, or awaiting
          verification.
        </p>
      </div>
                ...
                return (
                  <tr key={item.id} className="border-b border-border last:border-0 hover:bg-[#fafbfc]">
                    <td className="px-3.5 py-3">
                      ...
                      {status === "pending_approval" && item.over_budget_reason && (
                        <div className="mt-1 text-xs text-text-subtle">
                          Awaiting an executive — reason: {item.over_budget_reason}
                        </div>
                      )}
                    </td>
